...
mangoo I/O ships with an utility class for easy access of some configuration values.

[source,java]
----------------
ConfigUtils.getApplicationName();
----------------
...
in a fluent. You may know this feature as "Sign in with ...". The OAuth feature
...
in you application. You also have to connect the via OAuth authenticated user to your user mangement. This can, for example, done
by storing the OAuth id of the user along with the OAuth provider (@twitter, @goolge or @facebook). But how you handle this
is completily up to you.
...
mangoo I/O uses https://logging.apache.org/log4j/2.x/[Log4j2] for logging, enabling you
a variate of loggers to append to. If you are familiar with Log4j2,
...
src/main/resources. If you want to pass a custom log4j2.xml for e.g. different environments
you can simply set a "log4j.configurationFile" system property which will be picked up by Log4j2 and 
overwrite the appenders configured in the log4j2.xml file located in src/main/resources.
...
* https://github.com/svenkubiak/mangooio-email-extension
