...
You will be prompted for your group and artifact id for your new
project. You also have to set an application name and an application secret.
Make your secret "secure", as some functions, like sessions and authentication rely on it.
...
|_| |_| |_| \__,_||_| |_| \__, | \___/  \___/  |___|/_/    \___/ 
                          |___/                                  
https://mangoo.io | @mangoo_io | 1.0.0-RC5

mangoo I/O application started @127.0.0.1:8080 in 437 ms in dev mode. Enjoy.
---------------------------------------------------------------------------------------------------------

[NOTE]
Whenever you see the mangoo I/O logo the application has started successfully.
...
As mangoo I/O relies on Java 8, it is important that the files are compiled with the correct flags. To be more
precise, mangoo I/O relies on the parameter flag that enables easy lookup of method parameters.
...
If you don't do this, mangoo I/O will not handle passing request parameters at all in dev mode.
...
└── src
    └── main
        ...
        │   │   └── Routes.java                     //Configuratio for HTTP requests to classes and methods
        │   └── controllers
        │       └── ApplicationController.java      //Optional example controller in an optional package
        └── resources
            ├── application.yaml                    //Main application configuration
            ├── ehcache.xml                         //Cache configuration
            ├── files                               //Folder for serving static files (e.g. assets, robots.txt)
            ├── logback.prod.xml                    //Optional logback configuration for prod mode
            ├── logback.xml                         //Default logback configuration
            ├── templates                           //Templates folder
            │   ├── ApplicationController
            │   │   └── index.ftl
            │   └── layout.ftl                       //Base layout for all templates
...
The Module class is used for you custom Google Guice bindings and the Routes
...
information on serving static files. The /templates folder contains all templates
...
provided mangoo I/O test utilities. The prod mode is activated by default when no
other mode is fiven. You can overwrite this programatically, by setting a system property
...
It is recommended to use the non-prefixed values for prod configuration
and overwrite this values with mode specific values, keeping the configuration
values to a minimum.
...
requests to controllers classes and their methods. Whether if you render a
...
NOTE: Double and Float values are always passed with "." separation, either if you pass the query or request parameter with "," separation.
...
--------------------------------------------------------
form.required={0} is required
form.min={0} must be at least {1} characters
form.max={0} can be max {1} characters
form.exactMatch={0} must exactly match {1}
form.match={0} must match {1}
form.email={0} must be a valid eMail address
form.ipv4={0} must be a valid IPv4 address
form.ipv6={0} must be a valid IPv6 address
form.range={0} must be between {1} and {2} characters
form.url={0} must be a valid URL
--------------------------------------------------------
...
You can either obtain a prefilled hidden input field for your form simply
the token itself.
...
mangoo I/O uses http://ehcache.org/[Ehcache] for storing and accessing values
...
This class is not scheduled or executed at all right now, you have to
...
can use the application.yaml to pass any options to quartz as you
...
To retrieve JSON send your mangoo I/O you have three options:
automatic object serialization, generic object convert or the raw JSON
string.
...
can also work with the raw JSON content. The body object contains the raw
...
mangoo I/O offers some convenient functions to make sending eMails as
...
[NOTE]
You dont' have to create any SMTP configuration programmatically. This is
all done via the application.yaml of your mangoo I/O application. So you set your host,
port, username, password and ssl usage only once.
...
Freemarker template you can use the mangoo I/O TemplateEngine.
...
To access translation in a template, you can us a special tag with the key
...
There are two types of filters in mangoo I/O: Controller/Method filtera
and a global filter.
...
showing you if how to interact with the Undertow request/response cycle.
...
session cookie, set the following propertiy in your application.yaml
...
and store this hash value with the user data. This hashed value can be created with
...
the default templates of your application. Thus, there is no need for
...
mangoo I/O ships with some convenient tools for testing your application.
...
application.yaml and their default values.
