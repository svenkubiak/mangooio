...
mangoo I/O uses Google Guice for dependency injection, which enables you
to simple inject a class by calling
...
Guice will do the rest for you.

While it is possible to use member injection, it is recommended to do
constructor based injection, like this

[source,java]
------------------------
@Inject
public MyClass (MyMember myMember) {
...
The manager offers some convinient methods for postpone task into the background of
