...
import io.mangoo.routing.Secure;
...
public class Bootstrap implements MangooBootstrap {
    ...
    public void initializeRoutes() {
        ...
        // FilterController
        Bind.controller(FilterController.class).with(
        ...
        // I18nController
        Bind.controller(I18nController.class).with(
        ...
        // FlashController
        Bind.controller(FlashController.class).with(
                On.get().to("/flash").respondeWith("flash"),
                On.get().to("/flashed").respondeWith("flashed")
        );
        
        // JsonController
        Bind.controller(JsonController.class).with(
        ...
        // FormController
        Bind.controller(FormController.class).with(
        ...
        Bind.controller(AuthenticationController.class).with(
        ...
        // ParameterController
        Bind.controller(ParameterController.class).with(
         ...
         Bind.controller(ApplicationController.class).with(
                ...
                On.get().to("/limit").respondeWith("limit").maxRequestsPerSecond(10),
                ...
                On.put().to("/jsonboonpost").respondeWith("jsonBoonPut"),
                On.get().to("/freemarker").respondeWith("freemarker")
         );
         
         // SubController
         Bind.controller(SubController.class).with(
                 On.get().to("/subcontroller").respondeWith("check")
         );
         
         // BasicAuthenticationController
         Bind.controller(BasicAuthenticationController.class).with(
                 On.get().to("/basicauth").respondeWith("basicauth")
         );
         
         Bind.serverSentEvent().to("/sse");
         Bind.serverSentEvent().to("/sseauth");
         
         Bind.webSocket().onController(WebSocketController.class).to("/websocket");
         Bind.webSocket().onController(WebSocketController.class).to("/websocketauth");

         Bind.pathResource().to("/assets/");
         Bind.fileResource().to("/robots.txt");
         
         Secure.controller(BasicAuthenticationController.class).withBasicAuthentication("username", "password");
         Secure.method(BasicAuthenticationController.class, "foo").withBasicAuthentication("username", "password");
         
         Secure.url("/protected").withBasicAuthentication("username", "password");
         Secure.url("/dashboard").withAuthentication();
         Secure.urls("/dashboard", "bla", "bar", "foo").withAuthentication();
         Secure.url("/auth/*").withAuthorization("rolename");
    }
